...
export const usePermissions = () => {
  const { user, hasRole, isGroupAdmin, isGroupMember, canPerformAction } =
    useAuth();
  ...
  const checkPermission = (permission, context = {}) => {
    ...
    switch (permission) {
      ...
      case "EDIT_POST":
        const canEdit =
          user.id === context.authorId || user._id === context.authorId;
        return canEdit;

      case "DELETE_POST":
        const isAuthor =
          user.id === context.authorId || user._id === context.authorId;
        const isGroupAdminForPost =
      ...
      case "EDIT_PROFILE":
        const isProfileOwner =
          user.id === context.profileOwnerId ||
          user._id === context.profileOwnerId;
        const isAdmin = user.role === "admin";
        return isProfileOwner || isAdmin;

      case "MANAGE_PROFILE_SETTINGS":
        const isSettingsOwner =
          user.id === context.profileOwnerId ||
          user._id === context.profileOwnerId;
      ...
      case "MANAGE_GROUP":
        const isGroupAdminUser =
          context.admin === user._id || context.groupAdminId === user.id;
