...
export default function MangeGroupModal({
  group,
  isOpen,
  onClose,
  onGroupUpdated,
  ...
  const fetchPendingRequests = async () => {
    try {
      setLoading(true);
      const res = await axios.get(
        `http://localhost:3001/api/groups/${group._id}/requests`,
        {
  ...
  const fetchWatchlist = async () => {
    try {
      setLoading(true);
      const res = await axios.get(
        `http://localhost:3001/api/groups/${group._id}/watchlist`,
        {
          headers: { "x-auth-token": token },
        }
      );
  ...
  const handleAcceptRequest = async (userId) => {
    await axios.put(
      `http://localhost:3001/api/groups/${group._id}/requests/${userId}/accept`,
      {},
      {
        headers: { "x-auth-token": token },
      }
  ...
  const handleRejectRequest = async (userId) => {
    await axios.put(
      `http://localhost:3001/api/groups/${group._id}/requests/${userId}/reject`,
      {},
      {
        headers: { "x-auth-token": token },
      }
  ...
  const handleRemoveMember = async (userId) => {
    ...
    await axios.delete(
      `http://localhost:3001/api/groups/${group._id}/members/${userId}`,
      {
        headers: { "x-auth-token": token },
      }
    );
  ...
  const handleRemoveWatchlistItem = async (tmdbId, tmdbType) => {
    await axios.delete(
      `http://localhost:3001/api/groups/${group._id}/watchlist/${tmdbId}/${tmdbType}`,
      {
        headers: { "x-auth-token": token },
      }
    );
          ...
          <div className="modal-body">
            ...
            {tab === "watchlist" && (
              <div>
                ...
                {watchlist.length === 0 && (
                  <div className="text-muted">No items in watchlist.</div>
                )}
                <ul className="list-group">
                  {watchlist.map((item) => (
                    <li
                    ...
                    >
                      <div>
                        {item.tmdbPosterPath ? (
                          ...
                          <span
                            className="rounded bg-secondary d-inline-flex align-items-center justify-content-center me-2"
                      ...
                      <button
                        className="btn btn-outline-danger btn-sm"
                        onClick={() =>
                          handleRemoveWatchlistItem(item.tmdbId, item.tmdbType)
                        }
                        disabled={loading}
                      >
                        Remove
