...
export default function AddPostModal({ isOpen, onClose, onPostCreated, groupId = null }) {
    ...
    const handleSelectMovie = (item) => {
        const selectedData = {
            ...
            tmdbPosterPath: item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : '',
...
const handleSubmit = async (e) => {
    ...
    const categories = [
        { id: 'review', label: 'Review',   icon: <i className="bi bi-star-fill"></i>},
        { id: 'recommendation', label: 'Recommendation', icon:  <i className="bi bi-hand-thumbs-up"></i>},
        { id: 'discussion', label: 'Discussion', icon:<i className="bi bi-chat-dots"></i> },
        { id: 'question', label: 'Question', icon: <i className="bi bi-question-circle"></i> },
        { id: 'news', label: 'News', icon: <i className="bi bi-newspaper"></i> },
        { id: 'general', label: 'General', icon:<i className="bi bi-pencil-square"></i> }
    ];
