...
export default function FriendsList({ currentUser, onViewProfile }) {
  ...
  const fetchFriends = async () => {
    ...
    try {
      ...
      //Use user from useAuth instead of localStorage
  ...
  const handleRemoveFriend = async (friendId) => {
    ...
    try {
      ...
      // Show info message
      alert(
        "Friend removed successfully! They have been removed from both friends lists."
      );
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="text-center p-4">
        ...
        <button className="btn btn-warning btn-sm" onClick={fetchFriends}>
          Try Again
        </button>
      </div>
    );
  ...
  return (
    <div className="friends-list">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5 className="text-white mb-0">Friends ({friends.length})</h5>
      ...
      {filteredFriends.length === 0 ? (
        searchTerm ? (
          ...
          <EmptyState
            ...
            description="Start connecting with people!"
            showButton={false}
          />
        )
      ) : (
