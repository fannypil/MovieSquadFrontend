...
export default function UserSearch({ 
  ...
  showSelectedBadge = false
   ...
   const filteredUsers = searchResults.filter(user => 
        !excludeUserIds.includes(user._id || user.id)
    )

    const isUserSelected = (user) => {
        return selectedUsers.some(selected => 
            (selected._id || selected.id) === (user._id || user.id)
        )
    }
    ...
    <div>
      ...
      <div className="mb-3">
        <input
          ...
          style={{ backgroundColor: '#3c3c3c', color: 'white', border: '1px solid #555' }}
        />
      </div>
          ...
          <div className="row">
            {filteredUsers.map(user => (
              <div key={user._id || user.id} className="col-md-6 mb-3">
                <div 
                  className={`card h-100 ${isUserSelected(user) ? 'border-success' : ''}`}
                  style={{ 
                    backgroundColor: isUserSelected(user) ? '#1a472a' : '#3c3c3c', 
                  ...
                  <div className="card-body p-3">
                    <div className="d-flex align-items-center">
                      ...
                      <div className="ms-2">
                        {variant === "invite" ? (
                          <button 
                            className={`btn btn-sm ${isUserSelected(user) ? 'btn-success' : 'btn-outline-primary'}`}
                            onClick={(e) => {
                              e.stopPropagation();
                              onUserSelect(user);
                            }}
                          ...
                          <AddFriendButton 
                            ...
                            onStatusChange={(status) => handleFriendStatusChange(user._id, status)}
                          />
                        )}
                      </div>
                    </div>
